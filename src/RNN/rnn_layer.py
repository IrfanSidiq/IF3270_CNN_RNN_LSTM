...
class SimpleRNN(object):
    def __init__(self, units: int, return_sequences: bool = False, go_backwards: bool = False, input_size: int = 1) -> None:
        ...
        self.U_t = np.zeros((units, input_size), dtype=np.float32)
        self.W_t = np.zeros((units, units), dtype=np.float32)
        self.b_xh = np.zeros((units, 1), dtype=np.float32)
    ...
    def forward(self, input_sequence: np.ndarray) -> np.ndarray:
        ...
        for i in loop_indices:
            ...
            term_input = np.dot(self.U_t, input_i_batch) # (units * input_size) * (input_size * 1)
            term_hidden = np.dot(self.W_t, current_h_state) # (units * units) * (units * 1)

            h_t = SimpleRNN.activation_function(term_input + term_hidden + self.b_xh)
...
class Bidirectional(object):
    def __init__(self, units: int, merge_mode: str = 'concat', return_sequences: bool = False, input_size: int = 1):
        ...
        self.forward_rnn = SimpleRNN(units=self.units_per_direction, return_sequences=True, go_backwards=False, input_size=self.input_size)
        self.backward_rnn = SimpleRNN(units=self.units_per_direction, return_sequences=True, go_backwards=True, input_size=self.input_size)
...
class Model(object):
    ...
    def load_weights_from_keras_model(self, keras_model_instance: tf.keras.Model):
        ...
        custom_weight_bearing_layers = [
            ...
            if isinstance(layer, (Bidirectional, SimpleRNN, Dense))
        ]
        ...
        for i, (custom_layer, keras_layer) in enumerate(zip(custom_weight_bearing_layers, keras_weight_bearing_layers)):
            try:
                if isinstance(custom_layer, Bidirectional) and isinstance(keras_layer, tf.keras.layers.Bidirectional):
